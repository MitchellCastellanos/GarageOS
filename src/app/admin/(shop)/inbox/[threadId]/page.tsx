    ...
    <div className="p-5">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-sm font-medium text-slate-900">
          {message.direction === "INBOUND" ? message.from : `Tú → ${message.to.join(", ")}`}
        </span>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {await Promise.all(
            message.attachments.map(async (att) => {
              let href = "#";
              try {
                href = await signedUrlForCommunicationAttachment(att.storageKey);
              } catch {
                // sin storage configurado — el link queda inerte
              }
              return (
                <a
                  key={att.id}
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center gap-1.5 text-xs text-blue-600 hover:underline border border-slate-200 rounded-lg px-2 py-1"
                >
      ...
      <span
        className={`inline-flex mt-2 px-2 py-0.5 rounded-full text-[11px] font-medium ${
    ...
    <div className="max-w-3xl space-y-6">
      <div>
        <h1 className="text-xl font-bold text-slate-900">
          {thread.client ? formatClientName(thread.client) : thread.subject || "Conversación"}
        </h1>
        <p className="text-slate-500 text-sm mt-1">{thread.subject || "Sin asunto"}</p>
      </div>
      ...
      {clientHistory.length > 0 && (
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          <h3 className="text-sm font-semibold text-slate-900 mb-3">Historial automatizado del cliente</h3>
          <ul className="space-y-2">
            {clientHistory.map((m) => (
              <li key={m.id} className="text-xs text-slate-500 flex items-center justify-between">
                <span>
                  {m.purpose ?? m.channel} — {m.subject ?? m.status}
                </span>
      ...
      <ReplyBox threadId={thread.id} defaultTo={defaultTo} defaultSubject={defaultSubject} />
    </div>
  );
}
