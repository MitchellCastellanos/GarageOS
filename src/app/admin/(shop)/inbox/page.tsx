...
export default async function InboxPage({ searchParams }: PageProps) {
  ...
  const threads = await listThreads(activeTab);
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-1 bg-slate-100 p-1 rounded-lg w-fit">
        {TABS.map((tab) => (
          <Link
            key={tab.value}
            href={tab.value === "OPEN" ? ADMIN.inbox : `${ADMIN.inbox}?status=${tab.value}`}
            className={[
      ...
      {threads.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
          ...
          <p className="text-slate-500 font-medium">
            {activeTab === "OPEN" ? "No hay conversaciones abiertas" : "No hay conversaciones archivadas"}
          </p>
        </div>
      ) : (
          ...
          <div className="divide-y divide-slate-100">
            {threads.map((thread) => {
              ...
              return (
                <Link
                  key={thread.id}
                  href={adminPath(`/inbox/${thread.id}`)}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-slate-900 text-sm truncate">
                      {thread.client ? formatClientName(thread.client) : thread.subject || "Sin asunto"}
                    </p>
                    <p className="text-slate-500 text-sm truncate">
                      {last?.subject || thread.subject || "Sin asunto"}
                    </p>
                  </div>
                  {last && (
