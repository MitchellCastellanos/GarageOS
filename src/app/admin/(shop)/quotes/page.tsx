      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            {quotes.length} {quotes.length !== 1 ? t.list.countSuffix : t.list.countSuffixSingular}
      ...
      {quotes.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
          ...
          <p className="text-slate-500 font-medium">
            {activeTab === "ALL"
              ? t.list.emptyAll
              : t.list.emptyStatus(t.status[activeTab as keyof typeof t.status] ?? activeTab)}
          </p>
          {activeTab === "ALL" && (
            <Link
              href={`${ADMIN.quotes}/new`}
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div className="hidden sm:grid grid-cols-[1fr_160px_120px_100px_110px] gap-4 px-5 py-3 bg-slate-50 border-b border-slate-200 text-xs font-medium text-slate-500 uppercase">
            <span>{t.list.colQuoteClient}</span>
            <span>{t.list.colVehicle}</span>
            <span>{t.list.colDate}</span>
            <span>{t.list.colStatus}</span>
            <span className="text-right">{t.list.colTotal}</span>
          </div>
