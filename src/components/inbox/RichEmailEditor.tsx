    ...
    <div className="bg-slate-100 p-4 sm:p-6 rounded-b-lg">
      <div className="mx-auto max-w-[560px] overflow-hidden rounded-xl bg-white shadow-sm">
        ...
        <div className="border-t border-slate-200 bg-slate-50 px-6 sm:px-10 py-5 text-center">
          <p className="m-0 text-[11px] leading-5 text-slate-400">Este correo fue enviado por {shopName}.</p>
          <p className="mt-2 mb-0 text-[10px] text-slate-300">Enviado con GarageOS</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-slate-300 bg-white focus-within:ring-2 focus-within:ring-blue-100 focus-within:border-blue-500">
      ...
      <div className="flex flex-wrap items-center gap-1 border-b border-slate-200 bg-slate-50 px-2 py-1.5">
        {!preview && (
          <>
            <button type="button" className={toolbarButton} onMouseDown={(e) => e.preventDefault()} onClick={() => command("bold")} disabled={disabled} title="Negrita"><Bold className="h-4 w-4" /></button>
            <button type="button" className={toolbarButton} onMouseDown={(e) => e.preventDefault()} onClick={() => command("italic")} disabled={disabled} title="Cursiva"><Italic className="h-4 w-4" /></button>
            <span className="mx-1 h-5 w-px bg-slate-200" />
            <button type="button" className={toolbarButton} onMouseDown={(e) => e.preventDefault()} onClick={() => command("insertUnorderedList")} disabled={disabled} title="Lista"><List className="h-4 w-4" /></button>
            <button type="button" className={toolbarButton} onMouseDown={(e) => e.preventDefault()} onClick={() => command("insertOrderedList")} disabled={disabled} title="Lista numerada"><ListOrdered className="h-4 w-4" /></button>
            <span className="mx-1 h-5 w-px bg-slate-200" />
            <button type="button" className={toolbarButton} onMouseDown={(e) => e.preventDefault()} onClick={() => command("undo")} disabled={disabled} title="Deshacer"><Undo2 className="h-4 w-4" /></button>
            <button type="button" className={toolbarButton} onMouseDown={(e) => e.preventDefault()} onClick={() => command("redo")} disabled={disabled} title="Rehacer"><Redo2 className="h-4 w-4" /></button>
          </>
        )}
