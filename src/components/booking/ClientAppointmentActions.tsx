...
export function ClientAppointmentActions({
  ...
  function handleConfirm() {
    ...
    startConfirm(async () => {
      ...
      if (!res.ok) {
        setError(typeof data.error === "string" ? data.error : t.manage.confirmError);
        return;
      }
  ...
  function handleCancel() {
    if (!confirm(t.manage.cancelConfirmPrompt)) return;
    setError(null);
    startCancel(async () => {
      ...
      if (!res.ok) {
        setError(typeof data.error === "string" ? data.error : t.manage.cancelError);
        return;
      }
  ...
  if (step === "confirmed") {
    return (
      <div className="text-center py-8 space-y-3">
        ...
        <h2 className="text-2xl font-bold text-slate-900">{t.manage.confirmedTitle}</h2>
        <p className="text-slate-600">{t.manage.confirmedBody}</p>
      </div>
    );
  }
  ...
  if (step === "cancelled") {
    return (
      <div className="text-center py-8 space-y-3">
        ...
        <h2 className="text-2xl font-bold text-slate-900">{t.manage.cancelledTitle}</h2>
        <p className="text-slate-600">
          {shop.phone ? (
            <>
              {t.manage.rescheduleCall}{" "}
  ...
  return (
    <div className="space-y-6">
      <p className="text-slate-700 text-center">{t.manage.hello(clientName)}</p>

      <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3 text-sm">
        <div>
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">{t.manage.service}</p>
          <p className="font-medium text-slate-900 mt-0.5">{title}</p>
        </div>
        <div className="flex items-start gap-2">
          <Calendar className="w-4 h-4 text-teal-600 mt-0.5 shrink-0" />
          <div>
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">{t.manage.dateTime}</p>
        ...
        {vehicleLabel && (
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">{t.manage.vehicle}</p>
        ...
        {mechanicName && (
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">{t.manage.mechanic}</p>
      ...
      {!canConfirm && !canCancel && status === "CONFIRMED" && (
        <p className="text-center text-sm text-slate-500">
          {t.manage.alreadyConfirmedNotice}
          {shop.phone ? (
            <>
              {t.manage.alreadyConfirmedAt}
              <a href={`tel:${shop.phone}`} className="text-teal-700 font-medium">
                {shop.phone}
              </a>
            </>
